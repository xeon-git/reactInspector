...
(function() {
    ...
    const createSearchMenu = () => {menuOpen = true;
        ...
        const title = document.createElement('h2'); title.id = 'menu-header'; title.innerText = 'BlurStyle Debug';
        ...
        const checkboxes = document.querySelectorAll('#menu-checkbox-container input[type="checkbox"]');
    ...
    const closeMenu = () => {
        ...
        if (background && background.parentNode === document.body) {
    ...
    const performSearch = (searchCriteria, depth) => {
        ...
        if (foundComponents.length > 0) {displayComponents(foundComponents, useIndexPath);} else {console.log(`%cкомпонент "${searchCriteria}" не найден`, 'color: #ff6347; font-weight: bold;');}
    ...
    const displayComponents = (components, useIndexPath) => {
        components.forEach((component, index) => {
            ...
            if (useIndexPath && component.indexPath) {console.log(`%cпуть из индексов: %c${component.indexPath}`, 'color: #20B2AA; font-weight: bold;', 'color: #ffffff;');}
            console.log(`%cзначение:`, 'color: #ffa07a; font-weight: bold;', component.value);
            console.groupEnd();});
    };
    ...
    const getComponents = (obj, visited = new Set(), maxDepth = Infinity, currentDepth = 0) => {
        ...
        const traverse = (obj, path = '', indexPath = '', depth = 0) => {
            ...
            for (let key in obj) {
                if (typeof obj[key] === 'object' && obj[key] !== null) {
                    ...
                    if (obj[key]?.constructor?.$metadata$?.simpleName) {
                        const componentName = obj[key].constructor.$metadata$.simpleName;
                    ...
                    traverse(obj[key], componentPath, componentIndexPath, depth + 1);
                }
                childIndex++;
            }
        };
        ...
        const traverse = (obj, path = '', depth = 0) => {
            ...
            for (let key in obj) {
                if (typeof obj[key] === 'object' && obj[key] !== null) {
                    ...
                    if (Object.values(obj[key]).includes(valueToFind)) {
                    ...
                    traverse(obj[key], componentPath, depth + 1);
                }
            }
        };
        ...
        const traverse = (obj, path = '', indexPath = '', depth = 0) => {
            ...
            for (let key in obj) {
                if (typeof obj[key] === 'object' && obj[key] !== null) {
                    ...
                    if (componentPath.includes(searchTerm) || key.includes(searchTerm)) {
                        foundComponents.push({
                            name: key,
                            value: obj[key],
                            path: componentPath,
    ...
    const getComponentByPattern = (root, pattern) => {
        ...
        const traverse = (obj, depth = 0, path = [], visited = new Set()) => {
            if (visited.has(obj) || depth > pattern.depth || typeof obj !== 'object' || obj === null) return;
            visited.add(obj);
            ...
            for (let key in obj) {
                if (typeof obj[key] === 'object' && obj[key] !== null) {
                    const patternEntry = pattern.structure.find(p => p.name === key);
                    if (patternEntry) {
                        if (!result.namedComponents[patternEntry.name]) {result.namedComponents[patternEntry.name] = [];} result.namedComponents[patternEntry.name].push({component: obj[key], path: [...path, key]});}
                    traverse(obj[key], depth + 1, [...path, key], visited);}}};

        traverse(root);

        if (result.components.length > 0) {console.groupCollapsed(`%c${pattern.componentName}`, 'color: rgba(32, 255, 0, 1)');
            result.components.forEach(({ component }) => {
                ...
                pattern.structure.forEach((childPattern) => {
                    ...
                    if (child === null && childType === 'null') {console.log(`%c${childIndex} потомок (null):`, `color: ${color}`, child);}
                     ...
                     else {console.log(`%c${childIndex} потомок (${typeof child}):`, `color: ${color}`, child);}
                        ...
                        actions.forEach(action => {
                            if (action.battle) {
                                ...
                                if (pattern) {
                                    ...
                                    console.log(`%cавтоматическое применение сценария для: ${pattern.componentName}`, 'color: rgba(32, 255, 0, 1)'); components.forEach(({ component }) => childActions(component, action));}}});}
            ...
            if (event.keyCode === action.keyCode) {event.preventDefault();
                ...
                if (pattern) {
                    ...
                    if (components.length > 0) {console.log(`%cприменение сценария для: ${pattern.componentName}`, 'color: rgba(32, 255, 0, 1)'); components.forEach(({ component }) => childActions(component, action));}
                     else {console.log(`%c${pattern.componentName} недоступен для изменений`, 'color: rgba(255, 0, 0, 1)');}}}});
    ...
    const applyLoopValues = (component, childKey, loopValue) => {
        ...
        if (rangeMatch) {
            ...
            const stepValue = parseFloat(rangeMatch[4]);
            delayTime = parseInt(rangeMatch[5], 10);
            ...
            const loop = () => {
                ...
                component[childKey] = values[valueIndex];
                valueIndex = (valueIndex + 1) % values.length;

                const timeoutId = setTimeout(loop, delayTime); activeLoops.push(timeoutId);};
